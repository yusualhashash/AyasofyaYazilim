...
const CountrySelector = ({
  searchText = 'Find',
  searchEmptyValue = 'No countries found',
  defaultValue,
  countries = [{
    cultureName: 'de-DE',
    uiCultureName: 'de-DE',
  ...
  const filteredCountries = useMemo(() => {
    return filter ? countries.filter(country => country.displayName.toLowerCase().includes(filter.toLowerCase())) : countries;
  }, [filter, countries]);

  function onSelect(currentValue) {
  ...
  const SelectedCountry = ({
    ...
    {showLabel && <Text className="text-base mr-2.5">{displayName}</Text>}
    {flagIcon && <Image
        className="w-9 h-6"
        source={{ uri: `https://flagcdn.com/w320/${flagIcon}.png` }}
    />}
    </View>
  );
  ...
  return (
    <View className="flex  justify-items-center items-center">
      <TouchableOpacity className="justify-between rounded" onPress={() => setOpen(true)} >
      <SelectedCountry displayName=""  showLabel="" flagIcon="tr" {...(countries.find(country => country.cultureName?.toLowerCase() === value.toLowerCase()) || {})} />
      ...
      <Modal visible={open} animationType="slide" onRequestClose={() => setOpen(false)}>
        <View className="flex-1 p-5 bg-white">
          <TextInput
            className="p-2.5 mx-4 rounded bg-gray-100"
          ...
          className="mt-2">
            {filteredCountries.length === 0 ? (
              <Text>{searchEmptyValue}</Text>
            ) : (
              filteredCountries.map((country, index) => (
                <TouchableOpacity key={index} onPress={() => onSelect(country.cultureName || '')} className=" py-">
                  <SelectedCountry {...country} showLabel={true} />
                </TouchableOpacity>
              ))
            )}
          </ScrollView>
